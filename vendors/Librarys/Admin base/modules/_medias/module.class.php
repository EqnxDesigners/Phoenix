...
class MEDIAS extends DB {
    ...
    public function getFormLstFormat() {
        if($this->getLstFormats()) {
            ...
            foreach ($this->getLstFormats() as $format) {
                $result .= '<li class="select-img-format" role="'.$format['width'].'x'.$format['height'].'"><input type="checkbox" name="format_'.$format['id'].'" value="1" checked> '.$format['format'].' ('.$format['width'].'x'.$format['height'].')</li>';
            }
            $result .= '</ul>';
        }
        return $result;
